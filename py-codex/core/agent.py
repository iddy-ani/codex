...
class Agent:
    ...
    def __init__(self, config: AppConfig, working_directory: str | Path | None = None):
        self.config = config
        self.working_directory = Path(working_directory) if working_directory else Path.cwd()
        # [Agent] Working directory set to: {self.working_directory}", file=sys.stderr)
        ...
        if azure_endpoint:
            # Use Azure OpenAI
            # [Agent] Using Azure OpenAI with endpoint: {azure_endpoint}", file=sys.stderr)
            ...
            # Use standard OpenAI
            print("[Agent] Using standard OpenAI API", file=sys.stderr)
    ...
    def _compress_history(self, max_tokens: int):  # max_tokens is not directly used in this simple compression
        ...
        if num_to_summarize > 0:
            ...
            # [Agent] Summarized {num_to_summarize} messages.", file=sys.stderr)
    ...
    def _execute_tool_implementation(
        ...
        """
        if self._cancelled:
            # [Agent] Tool execution cancelled: {tool_call.function.name}", file=sys.stderr)
        ...
        try:
            ...
            return f"Error: Invalid JSON arguments received for tool {function_name}: {e.msg}"
        ...
        # [Agent] Executing approved tool: {function_name}")

        # *** SAVE AND CHANGE TO WORKING DIRECTORY ***
        original_cwd = Path.cwd()
        try:
            # Change to the agent's working directory for tool execution
            os.chdir(self.working_directory)
            # [Agent] Changed to working directory: {Path.cwd()}", file=sys.stderr)
            ...
            if function_name in TOOL_REGISTRY:
                ...
                # *** ADD WORKING DIRECTORY TO TOOLS THAT SUPPORT IT ***
                if "working_directory" in valid_params:
                    call_args["working_directory"] = str(self.working_directory)
                elif "cwd" in valid_params:
            ...
            # [Agent] Restored working directory: {Path.cwd()}", file=sys.stderr)
    ...
    async def process_turn_stream(
        ...
        self._current_stream = None
        # Only clear pending_tool_calls if we're starting a new turn with a prompt
        # If continuing (prompt=None), preserve existing pending_tool_calls
        ...
        for attempt in range(MAX_RETRIES):
            ...
            try:
                # [Agent] Attempt {attempt + 1}/{MAX_RETRIES} sending request...", file=sys.stderr)
                ...
                async for chunk in stream:
                    ...
                    if finish_reason:
                        # [Agent] Stream chunk finished with reason: {finish_reason}", file=sys.stderr)
                            ...
                            try:
                                ...
                                if index in started_tool_call_indices:
                                    yield create_stream_event(
                                        ...
                                        tool_function_name=final_call.function.name,
                                        tool_arguments_delta=final_call.function.arguments  # Include complete args
                                    )
            ...
            # --- Error Handling within Retry Loop ---
                ...
                # [Agent] Attempt {attempt + 1} failed: {error_msg}", file=sys.stderr)
                ...
                if should_retry:
                    # [Agent] Retrying in {current_retry_delay:.2f} seconds...", file=sys.stderr)
                ...
                last_error = e
                # [Agent] Attempt {attempt + 1} failed: An unexpected error occurred: {e}", file=sys.stderr)
                traceback.print_exc(file=sys.stderr)
                if attempt == MAX_RETRIES - 1:
                    yield create_stream_event(type="error", content=f"Error: Max retries reached. Unexpected error: {e}")
                    return
                if attempt < 1:
                    # [Agent] Retrying unexpected error in {current_retry_delay:.2f} seconds...", file=sys.stderr)
                    await asyncio.sleep(current_retry_delay)
                    current_retry_delay = min(current_retry_delay * 2, MAX_RETRY_DELAY_SECONDS)
                    continue
                else:
    ...
    async def continue_with_tool_results_stream(
        ...
        # DON'T clear pending_tool_calls here - let process_turn_stream handle it properly
